...
The current implementation validates research metadata and source provenance. No shroud model, specimen, simulation result, or physical performance result has been produced.

## What to inspect first

1. Read the [2026-09-09 source-review decision](day3-source-review.md). It identifies what the accessible literature establishes and which competitor treatments remain unresolved.
2. Inspect [Experiment 01](experiment-01-rigid-defect-duct.md), especially the measurement-system and matched-thrust requirements.
3. Run the [README quick start](../README.md#quick-start). These are offline integrity checks after dependency installation, not aerodynamic calculations.
4. Read the [claim ledger](claim-ledger.md) and [disclosure boundary](../CONTRIBUTING.md#public-disclosure-boundary) before proposing stronger claims or adding implementation-sensitive geometry.
...
The acquisition ledger preserves all 499 raw day-2 records and the ten day-1 source entries. Fifty raw rows lack successful query-log support. A DOI match is not a complete recall measurement; identifiers do not imply independent studies or completed reading. Recall is deliberately null.
